...
class Block {
    ...
    public:
        ...
        Block(std::vector<std::pair<std::string, int>> &data, std::string previous_hash, uint64_t timestamp, std::optional<uint64_t> nonce)
        ...
        const std::optional<uint64_t> get_nonce() const { return nonce; }
        ...
        void set_nonce(const std::optional<uint64_t> &new_nonce) { nonce = new_nonce; }
